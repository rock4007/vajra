...
def test_heart_rhythm_detection():
    ...
    print("\n" + "="*100)
    print("TEST 1: HEART RHYTHM DETECTION")
    print("="*100)
    ...
    try:
        ...
        test_result["status"] = "[PASS] Heart Rhythm Detection"
        test_result["details"] = {
            "normal_rhythm": normal_rhythm,
            "abnormal_rhythm": abnormal_rhythm
        }
        print("\n  [PASS] Heart Rhythm Detection")
        
    except Exception as e:
        test_result["status"] = "[FAIL] Heart Rhythm Detection"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] Heart Rhythm Detection: {e}")
    
    TEST_REPORT["tests"].append(test_result)
    return "[PASS]" in test_result["status"]

def test_geolocation_verification():
    """Test 2: Geolocation Verification"""
    print("\n" + "="*100)
    print("TEST 2: GEOLOCATION VERIFICATION & ACCURACY")
    print("="*100)
    ...
    try:
        ...
        for i, location in enumerate(TEST_LOCATIONS, 1):
            ...
            location_tests.append({
                ...
                "status": "[PASS]" if accuracy_score > 0.90 else "[FAIL]"
            })
        
        test_result["status"] = "[PASS] Geolocation Verification"
        test_result["details"] = {"location_tests": location_tests}
        print("\n  [PASS] Geolocation Verification")
        
    except Exception as e:
        test_result["status"] = "[FAIL] Geolocation Verification"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] Geolocation Verification: {e}")
...
def test_sos_alert_dispatch():
    ...
    print("\n" + "="*100)
    print("TEST 3: REAL-TIME SOS ALERT DISPATCH")
    print("="*100)
    ...
    try:
        ...
        print(f"\n  [SEND] Sending SOS alert to backend...")
        ...
        try:
            ...
            if response.status_code in [200, 202]:
                test_result["status"] = "[PASS] SOS Alert Dispatch"
                print("\n  [PASS] SOS Alert Dispatch")
            else:
                test_result["status"] = "[WARN] SOS Alert Dispatch"
                print(f"\n  [WARN] SOS Alert returned status {response.status_code}")
            ...
            test_result["status"] = "[FAIL] SOS Alert Dispatch"
            test_result["details"] = {"error": str(e)}
        
    except Exception as e:
        test_result["status"] = "[FAIL] SOS Alert Dispatch"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] SOS Alert Dispatch: {e}")
    
    TEST_REPORT["tests"].append(test_result)
    return "[PASS]" in test_result["status"] or "[WARN]" in test_result["status"]

def test_emergency_services_dispatch():
    """Test 4: Emergency Services Dispatch (Fire, Ambulance, Police)"""
    print("\n" + "="*100)
    print("TEST 4: EMERGENCY SERVICES DISPATCH (Location-based)")
    print("="*100)
    ...
    try:
        ...
        test_result["status"] = "[PASS] Emergency Services Dispatch"
        test_result["details"] = {"dispatch_log": dispatch_log}
        print("\n  [PASS] Emergency Services Dispatch")
        
    except Exception as e:
        test_result["status"] = "[FAIL] Emergency Services Dispatch"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] Emergency Services Dispatch: {e}")
...
def test_whatsapp_alerts():
    ...
    print("\n" + "="*100)
    print("TEST 5: WHATSAPP ALERTS TO 2 MEMBERS")
    print("="*100)
...
Contact emergency services or the victim"""
        ...
        test_result["status"] = "[PASS] WhatsApp Alerts"
        test_result["details"] = {"whatsapp_alerts": whatsapp_alerts}
        print("\n  [PASS] WhatsApp Alerts")
        
    except Exception as e:
        test_result["status"] = "[FAIL] WhatsApp Alerts"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] WhatsApp Alerts: {e}")
...
def test_emergency_calls():
    ...
    print("\n" + "="*100)
    print("TEST 6: EMERGENCY CALLS TO 2 MEMBERS")
    print("="*100)
    ...
    try:
        ...
        test_result["status"] = "[PASS] Emergency Calls"
        test_result["details"] = {"call_alerts": call_alerts}
        print("\n  [PASS] Emergency Calls")
        
    except Exception as e:
        test_result["status"] = "[FAIL] Emergency Calls"
        test_result["details"] = {"error": str(e)}
        print(f"\n  [FAIL] Emergency Calls: {e}")
...
def test_backend_health():
    ...
    print("\n" + "="*100)
    print("TEST 7: BACKEND HEALTH & API STATUS")
    print("="*100)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            test_result["status"] = "[PASS] Backend Health"
            test_result["details"] = {"health": health_data}
        else:
            test_result["status"] = "[WARN] Backend Health"
            print(f"  Backend Status: [PARTIAL]")
        
        print("\n  [PASS] Backend Health")
        
    except requests.exceptions.RequestException as e:
        test_result["status"] = "[FAIL] Backend Health"
        test_result["details"] = {"error": str(e)}
        print(f"  [FAIL] Backend not responding: {e}")
        print("\n  [FAIL] Backend Health")
...
def generate_final_report():
    ...
    passed = sum(1 for t in TEST_REPORT["tests"] if "[PASS]" in t["status"])
    ...
    print(f"[STAT] Success Rate: {(passed/total)*100:.1f}%")
    print("="*100)
    ...
    print(f"\n[SAVE] Full report saved to: {report_file}")

def main():
    """Run all tests"""
    print("\n[INIT] Starting Full-Flagged Test Suite...")
    print(f"[BACKEND] URL: {BACKEND_URL}")
    print(f"[TIME] Start Time: {datetime.now()}")
